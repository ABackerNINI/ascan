...
void MFileV3::output_targets() {
    ...
    MRule *debug_rule = new MRule("debug");
    debug_rule->add_prerequisite(new MFilename(m_build_path + "debug.mode"));

    idx = m_executable.size() == 1 ? -1 : 1;
    for (size_t i = 0; i < m_executable.size(); ++i) {
        debug_rule->add_prerequisite(new MSimpleVariable(m_cfg.make_bin(idx++)));
    }
    add_component(debug_rule);
    add_component(new MBlankLine());
    ...
    MRule *release_rule = new MRule("release");
    release_rule->add_prerequisite(new MFilename(m_build_path + "release.mode"));
    idx = m_executable.size() == 1 ? -1 : 1;
    for (size_t i = 0; i < m_executable.size(); ++i) {
        release_rule->add_prerequisite(new MSimpleVariable(m_cfg.make_bin(idx++)));
    }
    add_component(release_rule);
    add_component(new MBlankLine());
}
...
void MFileV3::output_executable_details() {
    ...
    for (auto &exec : m_executable) {
        ...
        if (settings.flag_build_) {
            ...
            // OUT: $(TARGET1): $(OBJS1)
            rule->add_prerequisite(new MSimpleVariable(m_cfg.make_objs(idx)));
        }
...
void MFileV3::output_compile_to_objects() {
...
void MFileV3::output_mode_control() {
    // OUT:
    //    # MODE CONTROL
    //
    //    $(BUILD)/%.mode:
...
void MFileV3::output_mm_dependencies() {
    ...
    depend_rule->add_prerequisite(new MSimpleVariable("SRC"));
...
void MFileV3::output_part() {}

void MFileV3::output_gitignore() {
    /*
    if (m_flags & OPTION_B) {
...
void MFileV3::add_mkdir_build_cmd_if_option_b(MRule *rule) {
